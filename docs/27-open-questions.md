...
- whether any user-facing surface is needed for the fail-safe end of a wake execution when critical
  presentation capability (notifications/full-screen) is lost mid-execution; the invalidation itself
  is deliberate and is now diagnosable as the INVALIDATED terminal state (with repair-target detail)
  in the Wake Lab reliability journal
