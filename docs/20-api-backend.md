...
**Status:** architectural direction, not an implementation commitment for M0–M6.

Wake My Way does not need a backend to schedule, fire, snooze, or complete the base alarm experience. Do not scaffold cloud services before a real capability requires them.
...
- account/sync later
- aggregated learning later
- subscription identity later

The backend never schedules or triggers the Android Wake Occurrence.
...
Realtime voice transport is not selected by this decision. Vercel WebSockets are a measured M7 candidate alongside direct provider and LiveKit/RTC approaches.
...
Network synchronization is eventually consistent and never required for current alarm behavior.
...
Remote state must never disable the Alarm Kernel, bundled emergency alarm, stop/snooze controls, or critical local fallback.
...
- never hold current Wake Occurrence authority or irreplaceable wake state only inside a Function process
- design realtime WebSocket clients for reconnect if the Vercel route wins the M7 spike
