...
Alarm Kernel                         Vercel
Wake Runtime                         ├── web / landing
local audio                          ├── non-critical API
motion inputs                        ├── Wake Plan generation
critical snapshot                    ├── ephemeral voice/session auth
        │                            └── future sync/context
        │                                      │
        └──────── wake still works ────────────┘
...
The Android app is **not hosted on Vercel**. It is compiled, distributed, installed and executed on Android. Vercel is a candidate/selected host for non-critical backend and web workloads once those workloads actually exist.
...
> Given the same typed inputs and policy, does Wake make the correct deterministic behavioral decision?

This can be tested quickly and repeatedly on the JVM and in Wake Lab.

### Platform/reliability truth

> Did Android actually fire the occurrence, start audible output and present usable controls under real lifecycle/device conditions?
...
```text
Wake Policy       default / hard-snoozer / fixture
...
00:48 ORIENTING   activation criterion reached
00:57 FINISHED    Wake Success
```
...
Wake Lab is for behavioral development. It **does not validate `AlarmManager` delivery reliability**.
...
This matters for Wake My Way because Play-managed installation/policy/capability behavior can differ from ad-hoc local installs.
...
Wake Success / completion
snooze behavior
fallback level
subjective annoyance / perceived agency
```

The goal is not merely that the alarm fires. The product thesis is validated only if Wake My Way materially improves the user's ability to become active near the intended time without unacceptable annoyance/friction.

## Cloud/deployment architecture

## Vercel role

Vercel is the preferred initial deployment platform for **non-critical cloud/web workloads**, once M7 or another concrete cloud capability justifies creating them.
...
The backend remains optional for base wake behavior.

## Vercel Functions

Vercel Functions with Fluid compute are a good fit for I/O-heavy work such as:
...
As of September 2026, Vercel documents up to 30-minute Node.js/Python Function durations for eligible Pro/Enterprise Fluid-compute workloads. This capability is useful context but must not encourage unnecessarily long request lifetimes.

Reconfirm current platform limits when the backend is implemented.

## Realtime voice and WebSockets

As of September 2026, Vercel Functions support WebSocket connections in **Public Beta** on Fluid compute. This makes a Vercel WebSocket proxy/voice gateway technically possible.

It is **not a locked architecture decision** for realtime voice.

M7 must compare at least:
...
Because Vercel WebSocket support is beta and connections follow Function lifetime constraints, clients must be designed for reconnects if that route is selected. Never keep wake authority or irreplaceable session state only inside a Vercel Function process.
...
No cloud database is authoritative for the current Android Wake Occurrence.
...
Cloud failure may reduce personalization/realtime richness. It must not prevent local wake delivery, audible fallback, stop/snooze or deterministic Wake Runtime behavior.

## Source references / time-sensitive platform facts

Recheck these sources when implementing the corresponding milestone:

- Vercel WebSocket Public Beta (2026-06-22): https://vercel.com/changelog/websocket-support-is-now-in-public-beta
- Vercel Functions up to 30 minutes for eligible Fluid-compute workloads (2026-06-15): https://vercel.com/changelog/vercel-functions-can-now-run-up-to-30-minutes
