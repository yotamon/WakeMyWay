...
class WakeRuntimeNoResponseTest {
    ...
    @Test
    fun `initial speech completion advances an unresponsive session into engaging`() {
        ...
        val afterSilence = runtime.reduce(
            ...
            WakeInput.SilenceElapsed(id("silence"), Duration.ofSeconds(15)),
            policy,
        )

        assertEquals(WakePhase.ACTIVATING, afterSilence.snapshot.phase)
        assertEquals(1, afterSilence.snapshot.escalationLevel)
        assertTrue(WakeDirective.Speak(SpeechIntent.ReEngage(1)) in afterSilence.directives)
