...
class DebugRealtimeWakeConversation(
    ...
    override fun connect() {
        ...
        val config = settings.load() ?: run { emitFailure("not-configured"); return }
        validateBrokerUrl(config.brokerUrl)
        disconnectResources()
        val current = generation.incrementAndGet()
        networkExecutor.execute {
            runCatching { requestBrokerSecret(config) }
                .onSuccess { secret -> mainHandler.post { if (isCurrent(current)) startPeerConnection(secret, current) } }
    ...
    private fun startPeerConnection(secret: BrokerSecret, current: Long) {
        ...
        try {
            ...
            audioSource = source
            val microphone = factory.createAudioTrack("wmw-founder-wake-mic", source).apply { setEnabled(inputEnabled) }
            ...
            peer.createOffer(object : SimpleSdpObserver() {
                override fun onCreateSuccess(description: SessionDescription?) {
                    if (!isCurrent(current)) return
                    val offer = description ?: run { emitFailure("offer"); return }
    ...
    private fun exchangeSdpAsync(peer: PeerConnection, offer: SessionDescription, secret: BrokerSecret, current: Long) {
        networkExecutor.execute {
            runCatching { exchangeSdp(secret, offer.description) }
                .onSuccess { answer -> mainHandler.post {
    ...
    private fun dataChannelObserver(channel: DataChannel, secret: BrokerSecret, current: Long) =
        object : DataChannel.Observer {
            override fun onBufferedAmountChange(previousAmount: Long) = Unit
            override fun onStateChange() {
                if (!isCurrent(current) || channel.state() != DataChannel.State.OPEN) return
                if (!sendSessionConfiguration(channel, secret.voice)) { emitFailure("session-config"); return }
                readyState.set(true)
                mainHandler.post { if (isCurrent(current)) listener.onConversationReady() }
            }
            override fun onMessage(buffer: DataChannel.Buffer?) {
                ...
                data.get(bytes)
                val event = runCatching { JSONObject(String(bytes, StandardCharsets.UTF_8)) }.getOrNull() ?: return
    ...
    private fun handleServerEvent(event: JSONObject, current: Long) {
        when (event.optString("type").takeIf(EVENT_TYPE_PATTERN::matches) ?: return) {
            "input_audio_buffer.speech_started" -> {
                ...
                mainHandler.post { if (isCurrent(current)) listener.onUserSpeechStarted() }
            }
            "input_audio_buffer.speech_stopped" -> {
                ...
                if (duration >= MIN_USER_TURN_MS) {
                    mainHandler.post { if (isCurrent(current)) listener.onUserTurnObserved() }
                }
            }
            "output_audio_buffer.started" -> if (!responseAudible) {
                responseAudible = true
                mainHandler.post { if (isCurrent(current)) listener.onAssistantSpeechStarted() }
            }
            ...
            "response.done" -> {
                ...
                if (status == "failed" || status == "incomplete") emitFailure("response")
            }
    ...
    private fun finishAssistantAudio(current: Long, interrupted: Boolean) {
        ...
        mainHandler.post { if (isCurrent(current)) listener.onAssistantSpeechFinished(interrupted) }
    }
    ...
    private fun sendSessionConfiguration(channel: DataChannel, voice: String): Boolean {
        ...
        return sendEvent(channel, JSONObject().put("type", "session.update").put("session", session))
    }
    ...
    private fun sendEvent(channel: DataChannel, event: JSONObject): Boolean {
        ...
        return bytes.size <= MAX_CLIENT_EVENT_BYTES && channel.send(DataChannel.Buffer(ByteBuffer.wrap(bytes), false))
    }
    ...
    private fun peerObserver(current: Long) = object : PeerConnection.Observer {
        ...
        override fun onIceConnectionChange(newState: PeerConnection.IceConnectionState?) {
            if (isCurrent(current) && newState == PeerConnection.IceConnectionState.FAILED) emitFailure("ice")
        }
        override fun onConnectionChange(newState: PeerConnection.PeerConnectionState?) {
            if (isCurrent(current) && newState == PeerConnection.PeerConnectionState.FAILED) emitFailure("peer")
        }
    }
    ...
    private fun requestBrokerSecret(config: FounderRealtimeConfig): BrokerSecret {
        val connection = (URL(config.brokerUrl).openConnection() as HttpURLConnection).apply {
            requestMethod = "POST"; connectTimeout = NETWORK_TIMEOUT_MS; readTimeout = NETWORK_TIMEOUT_MS
            useCaches = false; doInput = true
        ...
        try {
            ...
            check(token.isNotBlank() && voice.isNotBlank() && callsUrl == OPENAI_REALTIME_CALLS_URL)
    ...
    private fun exchangeSdp(secret: BrokerSecret, offerSdp: String): String {
        ...
        val connection = (URL(secret.callsUrl).openConnection() as HttpURLConnection).apply {
            requestMethod = "POST"; connectTimeout = NETWORK_TIMEOUT_MS; readTimeout = NETWORK_TIMEOUT_MS
            useCaches = false; doInput = true; doOutput = true
        ...
        try {
            connection.outputStream.use { it.write(offerSdp.toByteArray(StandardCharsets.UTF_8)) }
    ...
    private fun configureWakeAudioRoute() {
        ...
        audioManager = manager; previousAudioMode = manager.mode
        @Suppress("DEPRECATION") previousSpeakerphone = manager.isSpeakerphoneOn
        manager.mode = AudioManager.MODE_IN_COMMUNICATION
        @Suppress("DEPRECATION") runCatching { manager.isSpeakerphoneOn = true }
    }

    private fun restoreAudioRoute() {
        val manager = audioManager ?: return
        previousAudioMode?.let { runCatching { manager.mode = it } }
        previousSpeakerphone?.let { enabled -> @Suppress("DEPRECATION") runCatching { manager.isSpeakerphoneOn = enabled } }
        audioManager = null; previousAudioMode = null; previousSpeakerphone = null
    }

    private fun disconnectResources() {
        readyState.set(false); inputEnabled = false; responseAudible = false; speechStartedAtMs = null
        runCatching { dataChannel?.unregisterObserver() }; runCatching { dataChannel?.close() }; runCatching { dataChannel?.dispose() }; dataChannel = null
        ...
        runCatching { peerConnectionFactory?.dispose() }; peerConnectionFactory = null
        restoreAudioRoute()
    }

    private fun validateBrokerUrl(raw: String) {
        val uri = Uri.parse(raw)
        require(uri.scheme.equals("https", true) && !uri.host.isNullOrBlank() && uri.path.orEmpty().endsWith(FounderRealtimeSettings.FOUNDER_WAKE_PATH))
    }

    private fun readBounded(stream: InputStream, maxBytes: Int): String = stream.use { input ->
        val output = ByteArrayOutputStream(minOf(maxBytes, 16 * 1024)); val buffer = ByteArray(8 * 1024); var total = 0
        while (true) { val count = input.read(buffer); if (count == -1) break; total += count; check(total <= maxBytes); output.write(buffer, 0, count) }
    ...
    private fun emitFailure(stage: String) {
        ...
        mainHandler.post { if (isCurrent(current)) listener.onConversationFailure(stage.take(48)) }
    }
    ...
    private companion object {
        ...
        @Volatile var webRtcInitialized = false
