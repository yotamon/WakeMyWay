...
/**
 * Direct-distribution conversational Alfred setup.
 *
 * Infrastructure URLs, OpenAI credentials and WMW internal bearer secrets are intentionally absent
 * from this surface. The user enters one private access code once; the app exchanges it for an expiring
 * installation credential and stores that credential encrypted with Android Keystore.
 */
class RealtimeVoiceSetupActivity : ComponentActivity() {
    ...
    private val executor: ExecutorService = Executors.newSingleThreadExecutor { runnable ->
        Thread(runnable, "wmw-direct-realtime-pairing").apply { isDaemon = true }
    }
    ...
    private fun buildContent(): ScrollView {
        ...
        content.addView(TextView(this).apply {
            text = "During a conversational wake, live microphone audio and Alfred's generated audio use OpenAI Realtime. Wake My Way does not persist the audio or transcripts, and this Direct build does not send Tomorrow Contract or prepared private context to Realtime."
            textSize = 14f
            alpha = 0.75f
        })
    ...
    private fun renderIdle(status: FounderRealtimePairingClient.ServerStatus) {
        ...
        when {
            ...
            !configured && status.available -> {
                ...
                statusBody.text = "Enter the private access code once. Wake My Way handles the server and OpenAI connection automatically after that."
    ...
    private fun requestPairingConsent() {
        ...
        if (code.length < FounderRealtimePairingClient.MIN_ACCESS_CODE_LENGTH) {
            statusBody.text = "Enter the private access code first."
            return
        }
    ...
    private fun renderPairingFailure(error: Throwable) {
        ...
        when (pairingError?.kind) {
            FounderRealtimePairingClient.PairingException.Kind.ACCESS_CODE_REJECTED -> {
                ...
                statusBody.text = "Check the private access code and try again. No alarm settings were changed."
